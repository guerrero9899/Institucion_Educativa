...
<%
	...
	List<Resource> resources = (List<Resource>) query2.execute();
	...
	<div class="container-fluid text-center" style="margin-top: 30px">
		<div class="row content">
			...
			<div class="col-sm-8 text-left">
				...
				<form action="/access/add" method="get">
					<fieldset>
						...
						<select name="roleId">
							...
							%>
							<option value="<%=
							roles.get(i).getId()%>"<%if(i==0)out.print("selected"); %>><%=roles.get(i).getName()%></option>
							<%
								}
							%>
						</select> 
						<select name="resourceId">
							...
							%>
							<option value="<%=resources.get(i).getId()%>"<%if(i==0)out.print("selected"); %>><%=resources.get(i).getName()%></option>
							<%
								}
							%>
						</select> 
				...
				%>
				<p>No hay roles o resources creados</p>
				<% } %>
			</div>
		</div>
	</div>
